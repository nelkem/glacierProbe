                    ...
                    uint8_t numPairs)
{
  ...
  char* dweetString = "/dweet/for/%s?";
  memcpy(resource, dweetString, sizeof(dweetString));
  if (sizeof(resource) + sizeof(name) < 63) {
    memcpy((char*) &resource[sizeof(resource)], name, sizeof(name)); 
  }
  else {
#if DEBUG_MY4G
